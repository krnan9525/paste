...
To make an uncontrolled radio group, you do not pass the `value` or `onChange` props. If you need a radio to be checked by default, use the `defaultChecked` prop.

<LivePreview scope={{Radio, RadioGroup, useUID}} noInline>
  {uncontrolledRadioGroup}
</LivePreview>

To make a controlled radio group, you must pass the `value` and `onChange` props.
